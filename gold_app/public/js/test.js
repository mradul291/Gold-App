...
window.WBMComponents.payments = function ($mount, state) {
  ...
  let customerAdvanceBalance =
    state.payment_summary?.customer_advance_balance || 0;
  ...
  function fetchCustomerAdvance() {
    ...
    frappe.call({
      method:
        "gold_app.api.sales.wholesale_bag_melt.get_customer_advance_balance",
      args: { customer },
      callback: function (r) {
  ...
  $mount.on("click", "#add-payment", () => {
    ...
    const date =
      $mount.find("#manual-date-toggle").is(":checked") &&
      $mount.find("#manual-date").val()
