// Here Re Fetching the Wholesale Transection data from Api
...
// 				// Populate props and internal state from saved data
// 				this.props.buyer = data.buyer;
// 				this.props.buyer_name = data.buyer_name;
// 				this.props.sale_date = data.sale_date;
// 				this.props.totalAmount = data.total_cost_basis;

// 				this.bagSummary = data.receipt_lines || [];
// 				this.reconSummary = data.reconciliation_lines || [];
// 				this.adjustments = (data.adjustments || []).map((adj) => ({
// 					type: adj.adjustment_type || "",
// 					from_purity: adj.from_purity || "",
// 					to_purity: adj.to_purity || "",
// 					weight: adj.weight || 0,
// 					notes: adj.notes || "",
// 					impact: adj.profit_impact || 0,
// 				}));
// 			} else {
// 				// No existing record, fallback to defaults
// 				this.reconSummary = this.props.reconSummary.length
// 					? this.props.reconSummary
// 					: this.initializeReconSummary();
// 				this.adjustments = this.props.adjustments || [];
// 				this.bagSummary = [];
// 			}
// 		} catch (e) {
// 			// Fetch failed, fallback to defaults as well
// 			this.reconSummary = this.props.reconSummary.length
// 				? this.props.reconSummary
// 				: this.initializeReconSummary();
// 			this.adjustments = this.props.adjustments || [];
...
// 		// Then render and bind UI normally
// 		this.render();
...
// 	// Helper to check if adjustment type is weight loss
// 	isWeightLossType(adjustmentType) {
// 		return (
...
// 		const weightLossMap = {};
// 		this.adjustments.forEach((adj) => {
// 			if (this.isWeightLossType(adj.type)) {
// 				const purity = adj.from_purity;
// 				const weight = parseFloat(adj.weight) || 0;
...
// 				this.adjustments.forEach((adj, i) => {
// 					const optionHTML = adjustmentOptions
// 						.map(
// 							(o) =>
// 								`<option value="${o}" ${
...
//                     <td>${i + 1}</td>
//                     <td><select class="adjust-type">${optionHTML}</select></td>
//                     <td><input type="number" class="from-purity" placeholder="From" value="${
// 						adj.from_purity
// 					}" /></td>
//                     <td><input type="number" class="to-purity" placeholder="To" value="${
// 						adj.to_purity
// 					}" /></td>
//                     <td><input type="number" class="weight" placeholder="0" value="${
// 						adj.weight
// 					}" /></td>
//                     <td><input type="text" class="notes" placeholder="Enter notes or remarks..." value="${
// 						adj.notes
// 					}" /></td>
//                     <td class="profit-impact text-success">+RM ${adj.impact}</td>
//                     <td><button class="btn-delete-row" title="Remove">🗑️</button></td>
//                 </tr>
//             `);
// 					tbody.append(row);
...
// 			} else if (this.isWeightLossType(adjustmentType)) {
...
// 		tbody.on("input", ".from-purity", (e) => {
// 			const row = $(e.currentTarget).closest("tr");
// 			const adjustmentType = row.find(".adjust-type").val();
// 			if (this.isWeightLossType(adjustmentType)) {
// 				row.find(".to-purity").val(row.find(".from-purity").val());
// 			}
// 		});
// 	}
...
// Data from Reconcilation not receipt line
