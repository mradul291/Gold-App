  ...
  supplier: function (frm) {
    if (frm.doc.supplier) {
      frappe.db.get_doc("Supplier", frm.doc.supplier).then((supplier) => {
        ...
        const id_number =
          supplier.malaysian_id || supplier.other_id_number || "";
...
function hide_supplier_name_field(frm) {
  try {
    ...
    const selector =
      '[data-fieldname="supplier_name"], .form-group.field-supplier_name';
...
frappe.ui.form.on("Purchase Receipt", {
  supplier(frm) {
    ...
    // Ensure company is there; you can also default it if needed
    const company = frm.doc.company || frappe.defaults.get_default("company");

    frappe.call({
      method: "gold_app.api.purchase_receipt.get_supplier_advances_for_pr",
      args: {
      ...
      callback: function (r) {
        ...
        if (r.message && Array.isArray(r.message)) {
          r.message.forEach((row) => {
            let child = frm.add_child("supplier_advances");
            ...
            child.advance_amount = row.advance_amount;
            child.allocated_amount = row.allocated_amount;
            child.ref_exchange_rate = row.ref_exchange_rate;
            child.difference_posting_date = row.difference_posting_date;

            // if you have `account` in child doctype:
            if (row.account) {
