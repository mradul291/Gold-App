class WholesaleBagDirectPayment {
  ...
  loadCustomerAdvance() {
    ...
    frappe.call({
      method:
        "gold_app.api.sales.wholesale_bag_direct.get_customer_advance_balance",
      args: {
        customer: customerId,
      },
      callback: (r) => {
  ...
  updateAdvancePreview() {
    ...
    const amountInput =
      parseFloat($(this.containerSelector).find("#pay-amount").val()) || 0;
  ...
  addPayment() {
    ...
    if (method === "Customer Advance") {
      frappe.msgprint(
        "Please use the 'Use Advance' button for advance payments."
      );
      return;
    }
  ...
  addAdvancePayment() {
    ...
    const existingAdvance = this.payments.find(
    ...
    if (existingAdvance) {
      ...
      frappe.call({
        method:
          "gold_app.api.sales.wholesale_bag_direct.allocate_customer_advance_to_invoice",
        args: {
  ...
  removePayment(index) {
    ...
    if (removedPayment.method === "Customer Advance") {
      ...
      frappe.call({
        method:
          "gold_app.api.sales.wholesale_bag_direct.remove_customer_advance_allocation",
        args: {
          sales_invoice_name: refs.invoice_id,
        ...
        callback: (r) => {
          if (r.message && r.message.status === "success") {
            ...
            frappe.show_alert({
              message: `Removed RM ${removedPayment.amount.toLocaleString(
                "en-MY",
                {
                  minimumFractionDigits: 2,
                }
            ...
            frappe.msgprint(
              "Failed to remove advance allocation from invoice."
            );
          }
        },
        error: () => {
          frappe.msgprint(
            "Backend error while removing Customer Advance allocation."
          );
        },
      });
  ...
  async submitAllPayments() {
    ...
    if (!refs.invoice_id) {
      frappe.msgprint(
        "Sales Invoice not created. Please Create invoice first."
      );
      return;
    }
    ...
    try {
      ...
      const regularPayments = this.payments.filter(
        (p) => p.method !== "Customer Advance"
      );
      ...
      if (hasOnlyAdvancePayments) {
        const submitResult = await frappe.call({
          method:
            "gold_app.api.sales.wholesale_bag_direct.submit_sales_invoice_if_draft",
          args: {
            sales_invoice_name: refs.invoice_id,
          },
        });
        ...
        if (
          ...
          submitResult.message.status !== "success"
        ) {
          throw new Error(
            "Failed to submit Sales Invoice for advance-only payment."
          );
        }
      }
      ...
      for (const payment of regularPayments) {
        const result = await frappe.call({
          method:
            "gold_app.api.sales.wholesale_bag_direct.create_payment_entry_for_invoice",
          args: {
            sales_invoice_name: refs.invoice_id,
            ...
            posting_date: (() => {
              ...
              // Fallback: let backend use today's date
              return null;
            })(),
          },
        });
      ...
      if (refs.log_id) {
        await frappe.call({
          method:
            "gold_app.api.sales.wholesale_bag_direct.update_wholesale_bag_direct_payments",
          args: {
            log_id: refs.log_id,
