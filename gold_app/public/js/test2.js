...
// Added logic to Update - Profit Impact field data in Adjustment Section
class Step3TabReceiptReconciliation {
	constructor(props, container, backCallback, continueCallback, syncDataCallback) {
		this.props = props;
		this.container = container;
	...
	updateReconciliationSummary() {
		...
		(this.adjustments || []).forEach((adj) => {
			...
			`);
					...
					// Add to reconMap so rest of code recognizes it
		...
		try {
			...
			(this.adjustments || []).forEach((adj) => {
				...
				if (type === "Purity Blend (Melting)") {
					...
					// Calculate basic_rate (cost per gram)
					const basicRate = actualWeight > 0 ? costBasis / actualWeight : costBasis;
